...
class _FansState extends State<Fans> {
  ...
  void _loadInterstitialAd() {
    ...
    FacebookInterstitialAd.loadInterstitialAd(
      ...
      listener: (result, value) {
        print(">> FAN > Interstitial Ad: $result --> $value".tr());
  ...
  void initInterstitialAd() async{
    ...
    if(ads_interstitial_type == "ADMOB"){
      MobileAds.instance.initialize().then((InitializationStatus status) {
        print('Initialization done: ${status.adapterStatuses}'.tr());
        MobileAds.instance
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              if(statusList[index] == null){
                return TitleHomeWidget(title : "Fans Area".tr());
  ...
  Future<List<Status>>  _getList() async{
    ...
    else if (local == Locale("ar")) {
      lang = "ar_AE";
    }
    else if (local == Locale("he")) {
      lang = "he";
    }
    ...
    var statusCode = 200;
    try {
      response = await http.get(apiRest.statusByPage(page,lang));
    } catch (ex) {
      statusCode = 500;
    }
    if(!loading)
  ...
  statusLiked(Status status) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String  favoriteStatussString=  await prefs.getString('status_liked'.tr());
    ...
    if(liked_status == null){
      ...
      setState(() {
        status.liked = false;
        status.likes-=1;
        _toggleLike(status,"delete".tr());
      });
    }
    String encodedData = Status.encode(likedStatusList);
    prefs.setString('status_liked'.tr(),encodedData);
  }
  ...
  addShare(Status status) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    if (prefs.containsKey('shared_status_'.tr() + status.id.toString()) != true) {
      prefs.setBool('shared_status_'.tr() + status.id.toString(), true);
  ...
  addView(Status status) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    if (prefs.containsKey('viewed_status_'.tr() + status.id.toString()) != true) {
      prefs.setBool('viewed_status_'.tr() + status.id.toString(), true);
  ...
  addDownload(Status status) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    if (prefs.containsKey('downloaded_status_'.tr() + status.id.toString()) != true) {
      prefs.setBool('downloaded_status_'.tr() + status.id.toString(), true);
