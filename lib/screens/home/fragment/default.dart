...
class _DefaultState extends State<Default> {
    ...
    InterstitialAd.load(
        ...
        adLoadCallback: InterstitialAdLoadCallback(
          ...
          onAdFailedToLoad: (LoadAdError error) {
            print('InterstitialAd failed to load: $error.'.tr());
  ...
  Future<List<Post>> _getList() async {
    ...
    var response;
    print(apiRest.getHomeItems(lang));
    try {
      response = await http.get(apiRest.getHomeItems(lang));
    } catch (ex) {
      loading = false;
      setState(() {
        state = "error";
    ...
    if (response.statusCode == 200) {
      var data = await http.get(apiRest.getHomeItems(lang));
      ...
      SharedPreferences prefs = await SharedPreferences.getInstance();
      List<String> votedQuestions =
          prefs.getStringList('voted_question'.tr());
      ...
      String favoritePostsString = prefs.getString('post_favorires'.tr());
  ...
  Future<void> _loadMore() async {
    ...
    try {
      response = await http.get(apiRest.postByPage(page,lang));
    } catch (ex) {
      statusCode = 500;
    }
    if (!loading) return null;
