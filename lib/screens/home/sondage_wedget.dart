...
class _SondageWidgetState extends State<SondageWidget> {
  ...
  sendAnswers(){
    ...
    if(answers.isEmpty){
      Fluttertoast.showToast(
        msg:"Please select an answer before submit !".tr(),
  ...
  Future<String>  _submitAnswer(String answers_string) async{
    ...
    try {
      response = await http.post(apiRest.submitAnswer(), body: {'question'.tr(): widget.question.id.toString(),'choices': answers_string});
    } catch (ex) {
      statusCode =  500;
    }
    if(statusCode == 200){
      ...
      if(jsonData["code"] == 200){
        Fluttertoast.showToast(
          msg:"Your Answer has been counted successfully!".tr(),
          gravity: ToastGravity.BOTTOM,
  ...
  _saveSondage(int id ) async {
    ...
    if(!exist){
      ...
      prefs.setStringList("voted_question".tr(), votedQuestions);
    }
